...
#  --) buscar los lower bounds mediantes los sliders y que la func retorne la
...
class Detector(object):
    ...
    def add_img(self, path_to_img):
        self.original = cv2.imread(path_to_img)
        self.img = cv2.cvtColor(self.original, cv2.COLOR_BGR2HSV)
        self.img_extracted = self.original.copy()

    def nothing(self, x):
        pass

    def get_mask_bounds(self, path_to_img):
        mode = input('Set bound manually? [y/n]')
        if mode == 'y':
    ...
    def get_only_curve(self):
        ...
        try:
            ...
            cv2.waitKey()
            #cv2.imwrite('mascar.png', self.mask)

        except:
            print('Not enough contours found')


    def get_final_countours(self):
        img = cv2.imread('mascar.png')
        self.add_img('firma2.png')
        ...
        cv2.drawContours(self.original, contours, -1, (0, 0, 0), 2)
        cv2.imshow('Contours', self.original)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
